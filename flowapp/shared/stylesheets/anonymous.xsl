...
<xsl:template match="/">
	<table>
	  <tr>
		...
		<input type="hidden" name="requestType" value="followUp"/>
		<p>
			<input class="button" name="back" type="submit" value="{$BackLbl}" />
			<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
			<input class="button" name="next" type="submit" value="{$NextLbl}" />
		</p>
		</form>
	    </td>
	    <td width="100"><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></td>
